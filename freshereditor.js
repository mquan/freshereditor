...
(function ($) {
	...
	var methods = {
		...
		fontname: function() {
			...
			var height = $('.toolbar_fontname').width();
		...
		forecolor: function() {
			...
			var height = $('.toolbar_forecolor').width();
		...
		backcolor: function() {
			...
			var height = $('.toolbar_bgcolor').width();
			$('#colorpanel').css({"left": pos.left + 'px', "top": pos.top + height + 'px'});
			$('#colorpanel').show();		
		},
		...
		init: function (options) {
			...
			$.each(groups, function (index, commands) {
				...
				$.each(commands, function(i, command) {
					if(excludes.indexOf(command.name) < 0) { //lets developers exclude buttons
